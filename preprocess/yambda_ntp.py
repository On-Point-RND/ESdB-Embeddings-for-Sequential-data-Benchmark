...
def main():
    ...
    df = spark.read.parquet(args.data_path.as_posix())
    # -----------------------------
    # Заменяем null на -1 для числовых и array колонок
    for c, t in df.dtypes:
        if t in ("int", "bigint", "double", "float"):
            df = df.withColumn(c, F.coalesce(F.col(c), F.lit(-1)))
        elif t.startswith("array"):
            df = df.withColumn(
                c,
                F.when(F.col(c).isNull(), F.array(F.lit(-1))).otherwise(F.col(c))
            )
    # -----------------------------
    ...
    full_df = full_df.withColumn(
        "used_in_train",
        F.when(F.col("timestamp").isNotNull() & (F.size("timestamp") < 600),
               F.lit(-1)
               ).otherwise(F.lit(0))
    )
