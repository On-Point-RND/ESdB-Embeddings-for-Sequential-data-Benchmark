...
def main():
    ...
    df = spark.read.parquet(
        (args.data_path / "interactions.parquet").as_posix()
    )
    ...
    rows_df = collect_lists(
        df,
        group_by=INDEX_COLUMNS,  # одна строка на пользователя
        order_by=ORDERING_COLUMNS  # сортировка внутри последовательности
    )
    ...
    full_df = full_df.withColumn(
        "used_in_train",
        F.when(F.col(ORDERING_COLUMNS[0]).isNotNull() & (F.size(ORDERING_COLUMNS[0]) < 400),
               F.lit(-1)
               ).otherwise(F.lit(0))
    )
