...
def main():
    ...
    spark = (
        SparkSession.builder
            ...
            .config("spark.driver.maxResultSize", "0")  # без лимита на collect / first
    ...
    dates_df = (
        df.select("date").distinct()
            .orderBy("date")
            .withColumn("t", F.row_number().over(date_window) - 1)
            .persist()
    )

    T = dates_df.count()  # длина временного ряда

    # ---------- ADD TIME INDEX ----------
    df_idx = (
        df.join(dates_df, on="date", how="inner")
    )
    ...
    sales_sparse = (
        df_idx
            ...
            .agg(
            F.collect_list(
                F.struct("t", "unit_sales")
            ).alias("events")
        )
    )

    # ---------- DENSE ARRAY BUILD (ONCE) ----------
    full_time = spark.createDataFrame([(i,) for i in range(T)], ["t"])

    sales_dense = (
        sales_sparse
            .join(full_time, how="cross")
            .withColumn("unit_sales_dense",
                        F.coalesce(F.expr("filter(events, x -> x.t = t)[0].unit_sales"), F.lit(0.0))
                        )
            .groupBy("store_nbr", "item_nbr")
            .agg(F.collect_list("unit_sales_dense").alias("sales_ts"))
    )
    ...
    for c in pivot_df.columns:
        if c != "store_nbr":
            pivot_df = pivot_df.withColumnRenamed(c, f"unit_{c}_sales")
