...
CAT_FEATURES = ["currency", "operation_kind", "card_type", "operation_type", "operation_type_group",
                ...
                "day_of_week", "hour"]
INDEX_COLUMNS = ["app_id", "mock_target"]
...
def main():
    ...
    full_df = full_df.withColumn(
        "used_in_train",
        F.when(F.col("transaction_number").isNotNull() & (F.size("transaction_number") < 100),
               F.lit(-1)
               ).otherwise(F.lit(0))
    )
    ...
    train_df.repartition(25).write.parquet(
        (args.save_path / "train").as_posix(), mode=mode
    )
    test_df.repartition(25).write.parquet(
